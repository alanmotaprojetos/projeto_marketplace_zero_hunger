...
st.set_page_config(page_title='Restaurants View', layout='wide')
##===================================================================================
...
tab1, tab2 = st.tabs(['Marketplace - Restaurants View', '--'])

with tab1:
    with st.container():
        st.markdown("#### 4.1 - Top 10: Restaurant with the most reviews.")
        df_aux_4_1 = df.loc[:, ['Country Name', 'Restaurant Name', 'Votes']].groupby(['Country Name', 'Restaurant Name']).sum()
        df_aux_4_1 = df_aux_4_1.sort_values('Votes', ascending=False).head(10)
        df_aux_4_1.idxmax().head(1).reset_index()
        st.dataframe(data=df_aux_4_1, width=800, height=390, use_container_width=False)     

     
    st.markdown("""___""")    
    with st.container():
        st.markdown("#### 4.2 - Brazilian cuisine that have a recorded average rating( Rating >= 4.5).")
        culinaria_br_0 = (df['Country Name'] == 'Brazil') &  (df['Aggregate rating'] >= 4.5)
        df_aux = df.loc[culinaria_br_0, ['Country Name', 'Restaurant Name', 'Cuisines', 'Aggregate rating']]
        df_aux.sort_values('Aggregate rating', ascending=False).head(10)
        df_aux.reset_index().head(10)
        st.dataframe(data=df_aux, width=800, height=390, use_container_width=False)


    st.markdown("""___""")    
    with st.container():
        st.markdown("#### 4.3 - Brazilian cuisine that have the lowest average ratings( Rating = 0 ).")
        culinaria_br_0 = (df['Country Name'] == 'Brazil') &  (df['Aggregate rating'] == 0.0)
        df_aux = df.loc[culinaria_br_0, ['Country Name', 'Restaurant Name', 'Cuisines', 'Aggregate rating']]
        df_aux.sort_values('Aggregate rating', ascending=True).head(10)
        st.dataframe(data=df_aux, width=800, height=390, use_container_width=False)
